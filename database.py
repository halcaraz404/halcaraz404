...
def get_balance(user_account_number):
    ...
    try:
        ...
        for entry in user_data:
            ...
            if(entry == ","):
                count = count + 1
                if (count == 4):
                    balance = float(user_data[index:len(user_data)])
...
def track_login(user_account_number):

    try:

        f = open(user_log_path + str(user_account_number) + "_Login"+".txt", "x")
        ...
        f.write("User account number: " + user_account_number + ", logged in at: " + now.strftime("%Y-%m-%d at %H:%M:%S") + "\n")
...
def update_login_file(user_account_number, action):

    f = open(user_log_path + str(user_account_number) + "_Login"+".txt", "a")

    f.write("User account number: " + user_account_number + ", made a " + action + "  at: " + now.strftime("%Y-%m-%d at %H:%M:%S") + "\n")
